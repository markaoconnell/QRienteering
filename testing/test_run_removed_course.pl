...
###########
# Test 3 - Remove the course (hide it really)
# Validate that the correct entry is created
%TEST_INFO = qw(Testname RemoveWhiteCourse);
%GET = qw(key UnitTestPlayground remove:00-White on);
$GET{"event"} = $event_id;
...
###########
# Test 4 - find all correct controls
# Validate that the correct entry is created
